...
class DamwraSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for i in range(0,20,1):
            ...
            item['WaterLevel'] = res[9+12*i].replace(',','')
            item['EffectiveWaterStorageCapacity'] = res[10+12*i].replace(',','')
            item['PercentageUsedInReservoirCapacity'] = res[11+12*i].replace(',','').replace(' %','') 
            item['MaximumCapacity'] = res[1+12*i].replace(',','')
            yield item